...
- **GPU host integration:** `docs/GPU_HOST_INTEGRATION.md` — the handshake between the repo and the GPU desktop. **Working on the GPU desktop? That document names the outstanding job in its first section.** Read it before touching `OllamaGpuLlmProvider` too: it calls a `/v3` API that does not exist on that host
