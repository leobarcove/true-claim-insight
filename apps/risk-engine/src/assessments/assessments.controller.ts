...
export class AssessmentsController {
  ...
  async uploadAudio(@Req() req: FastifyRequest) {
    ...
    for await (const part of req.parts()) {
      this.logger.log(`Parsing part: type=${part.type}, fieldname=${part.fieldname}, encoding=${part.encoding}, mimetype=${part.mimetype}`);
      if (part.type === 'file') {
    ...
    this.logger.log(`Received audio upload for session ${sessionId}, size: ${audioBuffer.length} bytes`);
