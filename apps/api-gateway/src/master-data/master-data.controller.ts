...
export class MasterDataController {
  ...
  @Get('vehicles/makes')
  @Roles('FIRM_ADMIN', 'SUPER_ADMIN', 'ADJUSTER', 'SIU_INVESTIGATOR')
  @ApiOperation({ summary: 'Get all vehicle makes' })
  @ApiResponse({ status: 200, description: 'List of vehicle makes' })
  async getVehicleMakes(@CurrentTenantContext() tenant: TenantContext) {
  ...
  async getVehicleStructure(@CurrentTenantContext() tenant: TenantContext) {
  ...
  async createVehicleMake(
    @Body() createVehicleMakeDto: CreateVehicleMakeDto,
    @CurrentTenantContext() tenant: TenantContext
  ) {
  ...
  async deleteVehicleMake(@Param('id') id: string, @CurrentTenantContext() tenant: TenantContext) {
  ...
  async deleteVehicleModel(@Param('id') id: string, @CurrentTenantContext() tenant: TenantContext) {
