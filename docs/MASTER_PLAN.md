...
| `assertConsent` wired to processing | ⬜ **not done.** The method exists and is tested, but has no caller. The processing it should gate — biometric analysis of a video assessment — runs in video-service and risk-engine, which hold no consent access, so wiring it is an architectural step (a consent check those services can call) rather than a one-line change. Recorded rather than glossed: an unenforced gate is exactly the §3.6 pattern |
