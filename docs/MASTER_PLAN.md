...
| Final report ≤10 wkg days from complete documents | **PARTIAL** — clock wired (10 working days), pauses on awaiting-documents and SIU referral, stopped by issuing the FINAL report, breach + escalation verified live, operating on real dates. Remains PARTIAL for one honest reason: CSP runs the window from *complete documents*, and the clock still starts at `REPORT_PENDING` as a proxy because the evidence checklist emits no completeness event (`documentsCompleteAt`) yet | `documentsCompleteAt` event; the checklist-gating false-comfort item (§3.6 #8) closes with it | 1 |
...
- **Compliance:** matrix rows flip from FAIL/PARTIAL to PASS only with (a) server-side enforcement evidence and (b) a CI compliance test asserting the control. Current **15 PASS / 13 PARTIAL / 3 FAIL**, recounted from the table (never carried forward by hand) — the matrix is re-audited at each phase exit and the trend is the firm's readiness metric.
...
The two remaining FAILs are blocked, not pending: **AMLA/CTF screening** on counsel gate G8 (is a registered adjuster a reporting institution?), and **Sch 7 template governance** on the consent-wording legal review. Both are yours/counsel's to unblock; nothing in the codebase waits on me.
