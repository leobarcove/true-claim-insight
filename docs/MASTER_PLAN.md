...
- **Compliance:** matrix rows flip from FAIL/PARTIAL to PASS only with (a) server-side enforcement evidence and (b) a CI compliance test asserting the control. Current **14 PASS / 14 PARTIAL / 3 FAIL**, recounted from the table (never carried forward by hand) — the matrix is re-audited at each phase exit and the trend is the firm's readiness metric.
...
Re-audit note: the sweep caught a second malformed matrix row — the ITO-decision line carried **two verdicts** (`**FAIL** | **PARTIAL**`) from an earlier regex that replaced the wrong cell. The recount treated it as FAIL (first match), so the published counts were conservative rather than inflated — the right direction to be wrong in, but wrong. Repaired.
