...
| `Consent` entity + capture before processing | ✅ replaces `isPdpaCompliant`, the boolean the frontends set and nothing read (§3.6 item 4). Consent is now a record of *which wording* a named person agreed to, when, and whether it still stands. Three server-side gates: a notice version cannot be **approved** unless it exists in both English and Bahasa Malaysia (PDPA s.7); consent cannot be **recorded** against unapproved wording; processing that depends on consent calls `assertConsent` rather than reading a flag. Withdrawal retains the original grant, because evidencing *when* processing became unlawful needs both. Three purposes: claim processing, biometric analysis, cross-border transfer. **Draft EN/BM wording is seeded unapproved** — see the blocker below. 8 tests |
...
### Email provider: Amazon SES in Malaysia, not Resend
