...
The audit's most dangerous items are places where the system *claims* a control that does not run. Status as at the 31 July 2026 re-audit — **8 of 10 closed**:
...
- **Compliance:** matrix rows flip from FAIL/PARTIAL to PASS only with (a) server-side enforcement evidence and (b) a CI compliance test asserting the control. Current **8 PASS / 13 PARTIAL / 10 FAIL**, recounted from the table (never carried forward by hand) — the matrix is re-audited at each phase exit and the trend is the firm's readiness metric.
