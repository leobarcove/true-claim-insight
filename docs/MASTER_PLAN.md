...
| 12.3/12.4 | 12.3: *new* adjusting employees closely supervised by a senior for ≥1 year before independent work. 12.4: senior recognition = ≥5 years in the subject matter plus case volume/quality. (The <5-years *report countersign* is 12.7(b), not here) | **PARTIAL** — the countersign machinery exists, blocks in licensed mode and is asserted by 16 tests incl. the licence flip. It cannot yet key off real seniority: `AdjusterCompetency` is Phase 3, so unknown seniority is treated as junior (the safe default) and the basis is persisted on every report | Seniority derivation; supervised-status on assignments | 3 |
...
| 12.9–12.11 | Training; CPD 15 hrs/yr; programmes per the **Association of Malaysian Loss Adjusters** (12.11 — that AMLA is the association, not the Anti-Money Laundering Act; §3.4 and gate G8 use the Act. `Adjuster.amlaMember` means the association) | **FAIL** | `CpdRecord` ledger + annual floor dashboard | 3 |
...
| s.143 | Produce documents/information to BNM in specified form | **PARTIAL** — `GET /claims/:id/export` produces the complete machine-readable claim file: claim, claimant (NRIC decrypted — production to the regulator is what the record is kept for), appointment, documents *including soft-deleted ones*, reports, SLA history, consents, transfer records, the audit trail, sessions and notes. Completeness is data (`BUNDLE_SECTIONS`) asserted at assembly — a partial file refuses rather than presenting as complete — and every export writes a **sha256-sealed** audit row, so what was produced stays provable byte-for-byte. Compliance roles only; the read is itself audited at the gateway. Remains PARTIAL: document binaries and report PDFs are referenced by pointer, not packaged into one archive | 7 tests, in CI | done (archive packaging open) |
...
The audit's most dangerous items are places where the system *claims* a control that does not run. Status as at the 31 July 2026 re-audit — **7 of 10 closed**:
...
- **Compliance:** matrix rows flip from FAIL/PARTIAL to PASS only with (a) server-side enforcement evidence and (b) a CI compliance test asserting the control. Current **5 PASS / 15 PARTIAL / 11 FAIL**, recounted from the table (never carried forward by hand) — the matrix is re-audited at each phase exit and the trend is the firm's readiness metric.
...
### Seniority is the honest gap in the report engine
PD 12.3/12.7(b) keys the countersign requirement to whether the author has under five years in the subject, and `AdjusterCompetency` is Phase 3 — so that data does not exist. Rather than assume every author is senior (which would silently pass a junior's unsigned report) or assume none are (which would block a one-adjuster firm outright), `countersignDecision` treats unknown seniority as junior — the safe default, since wrongly requiring a countersign costs a signature while wrongly waiving one issues an unsupervised junior's report over the firm's name — and persists the basis on the report. When the competency model lands, the same function starts returning real seniority and nothing else changes.
