...
| 11 Aug 2026 | **Full re-audit, Tiers 1–6, against the original reports.** Thirty-one findings verified closed in code, not from notes. Three were still open and had been missed twice: `edited_message` was never in `allowed_updates`, so a claimant correcting a typo by editing got total silence — the same failure as the vanishing voice note, triggered by a more considerate claimant; only the *future* half of the date-range finding had been fixed, so `1026` for `2026` still validated and arrived pre-marked out of window, a claim rejected for one slipped digit; and five routes carried `@Throttle({ default: … })` against a config defining only `short`, `medium` and `long`, so the tightest limits in the system — five OTP sends an hour, ten logins a minute — had never applied to anything. That last one is not Telegram's and outlives this file: it protects the PWA login, and `throttler-names.spec.ts` now fails if any override names a throttler that does not exist. |
| 11 Aug 2026 | **Conversational re-audit — read as a claimant, not as code.** Six tiers of correctness work had never asked whether this is a *good bot*, and laying the whole script end to end answered differently. The bot opened by demanding identity and explaining why typing a number would not work — an implementation detail, in the first sentence someone reads after losing their luggage. It promised "a few questions and we are done" and then said "(1 of 16)", contradicting itself while trust was still being established. It never said what to gather, so a claimant met "upload the Property Irregularity Report" at question eleven with nothing to hand and had two options, both losses: abandon, or skip something the claim needs. It never said the conversation could be paused, though the state was always in the database. It asked three questions for a payout account, over Telegram, with no reassurance at all — the exact shape of a scam. And it drifted between "I" and "we", sometimes a person and sometimes a firm, which is precisely the inconsistency someone reads as a tell. All fixed; `whatYouWillNeed` derives the gather-list from the flow so an edited flow cannot promise the wrong one, and `conversation-script.spec.ts` pins the properties that only appear when the messages are read end to end. One finding was created by the Tier 2 work: serving the consent notice in Malay and then every question in English promises support that does not exist, so the opening now says so in Malay. |
